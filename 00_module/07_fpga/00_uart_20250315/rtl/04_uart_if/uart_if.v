...
`ifdef SIM
    ...
    input                               UART_RX_DVLD                            ,//(i)[   1]
    input       [   7:0]                UART_RX_DATA                            ,//(i)[   8]

    output                              UART_TX_REQ                             ,//(o)[   1]
    input                               UART_TX_ACK                             ,//(i)[   1]
    output       [   7:0]               UART_TX_DATA                            ,//(o)[   8]
`else
    input                               UART_RXD                                ,// (i) [   1]
    output                              UART_TXD                                ,// (o) [   1]
`endif
...
// =============================================================================
    ...
    wire                                s_BPS_RX_TRIG                           ;//(s)[  1]
    wire                                s_BPS_TX_TRIG                           ;//(s)[  1]

    wire                                s_UART_RX_ERR                           ;//(s)[  1]
    wire                                s_UART_RX_VLD                           ;//(s)[  1]
    wire        [   7:0]                s_UART_RX_DAT                           ;//(s)[  8]
    ...
    wire        [   7:0]                s_UART_TX_DAT                           ;//(s)[  8]

    wire                                s_FRAME_TIMEOUT                         ;//(s)[  1]
...
//      .UART_RX_BERR                   ( s_UART_RX_ERR         ),// (o) [   1]
...
`ifdef SIM
        ...
        .UART_RX_VLD                    ( UART_RX_DVLD          ),//(i)[   1]
        .UART_RX_DAT                    ( UART_RX_DATA          ),//(i)[   8]
`else
...
`endif
    ...
    RX_DEC U_RX_DEC (
        ...
        .REG_OP_ADDR                    ( s_REG_OP_ADDR         ) //(o)[  16]
         );
...
`ifdef SIM
        ...
        .UART_TX_DAT                    ( UART_TX_DATA          ) // (o) [   8]
`else
        .UART_TX_REQ                    ( s_UART_TX_REQ         ),// (o) [   1]
        .UART_TX_ACK                    ( s_UART_TX_ACK         ),// (i) [   1]
        .UART_TX_DAT                    ( s_UART_TX_DATA        ) // (o) [   8]
`endif
    );
