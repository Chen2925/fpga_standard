...
/+=============================================================================*/
    ...
    always @(posedge CLK or posedge RST)begin
        if(RST)begin
            r_SPI_OP_ADDR               <= 'b0 ;
        end else if(s_SPI_CLK_RISE & (r_SPI_BIT_CNT < 5'd7)) begin
    ...
    always @(posedge CLK or posedge RST)begin
        if(RST)begin
            r_SPI_WR_FLAG               <= 1'b0 ;
            r_SPI_RD_FLAG               <= 1'b0 ;
        end else if(s_SPI_CLK_RISE & (r_SPI_BIT_CNT == 5'd8) & (~SPI_GP_MOSI)) begin
            r_SPI_WR_FLAG               <= 1'b1 ;
        end else if(s_SPI_CLK_RISE & (r_SPI_BIT_CNT == 5'd8) & (SPI_GP_MOSI))begin
            r_SPI_RD_FLAG               <= 1'b1 ;
        end
    ...
    always @(posedge CLK or posedge RST)begin
        if(RST)begin
            r_SPI_RD_EN                 <= 1'b0 ;
        end else if(s_SPI_CLK_RISE & (r_SPI_BIT_CNT == 5'd7) & (SPI_GP_MOSI))begin
            r_SPI_RD_EN                 <= 1'b1 ;
        end else begin
            r_SPI_RD_EN                 <= 1'b0 ;
        end
    ...
    always @(posedge CLK or posedge RST)begin
        if(RST)begin
            r_SPI_WR_DATA               <= 'b0 ;
        end else if(s_SPI_CLK_RISE & (r_SPI_BIT_CNT > 5'd8)) begin
            r_SPI_WR_DATA               <= {r_SPI_WR_DATA[14:0],SPI_GP_MOSI} ;
        end
    end

    assign s_SPI_EOP                    = ((r_SPI_BIT_CNT == 5'd24) & s_SPI_CLK_FALL)  ;
